...
int createHTree(HTNode ht[], long *w)
{
    ...
    while (i < m - numZero)
    {
        ...
        ht[i].lchild = ht + pos1; //小权值为左孩子 //ps
        ht[i].rchild = ht + pos2; // ps
        ht[i].w = min1 + min2;
        ht[pos1].parent = ht[pos2].parent = ht + i; // ps
        i++;
    }
...
unsigned char *parseFile(const char filename[], long *w, long *fsize)
{
    ...
    fseek(fp, 0, SEEK_END); // ps 将 fp 定位到文件末尾
    *fsize = ftell(fp);     // ps 计算 fp 相对于文件头的偏移量
    rewind(fp);             // ps 将 fp 定位回文件头
...
void getCode(HTNode ht[], int c, unsigned *code1, int *clen1)
{
    ...
    int parent = ht[c].parent - ht; // ps
    while (parent >= 0)
    {
        code = code >> 1;
        // ps
        ht[parent].lchild - ht == c ? code : (code = code | mask); //如果是右孩子，那么最高位设置为1，否则就是0
        clen++;
        c = parent;
        parent = ht[c].parent - ht; // ps
    }
...
void encode(unsigned char *orgi, long olen, unsigned char *newc, long *nlen, HCode hc[])
{
    ...
    {                      //对原始数据逐个字符处理
        ...
        code = code & ((1 << len) - 1);    // code和后面len个1，其它全为0的数据进行与，保留后面len位
        code = code << 8 - len - usedbits; //向前移动若干位,移动至最前面
        newc[j] = newc[j] | code;//刷新newc[j]
        usedbits = len + usedbits;//已经用过的bits数刷新
        i++;//遍历
    }
    *nlen = j + 1;//纪录长度
}
...
void zip(char fin[], char fout[])
{
    ...
    HTNode *deta = ht;                      // ps
    fwrite(&deta, sizeof(HTNode *), 1, fp); // ps 保存数组首地址，用于解码时计算偏移量
...
void unzip(char zfile[], char ofile[])
{
    ...
    // ps
    HTNode *deta;
    fread(&deta, sizeof(HTNode *), 1, fp); // ps Huffman树的偏移量
    ...
    {
        if (ht1[i].parent)
            ht1[i].parent += ht1 - deta;
        if (ht1[i].lchild)
            ht1[i].lchild += ht1 - deta;
        if (ht1[i].rchild)
            ht1[i].rchild += ht1 - deta;
        // printf("")
    }
    int idx = root;      //从根开始
